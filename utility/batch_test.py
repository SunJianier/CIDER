...
def ranklist_by_heapq(user_pos_test, test_items, rating, Ks):
    ...
    K_max = max(Ks)
    K_max_item_score = heapq.nlargest(
        K_max, item_score, key=item_score.get
    )  # 取出候选item中排名前K的item id
            ...
            for i_batch_id in range(n_item_batchs):
                ...
                if drop_flag == False:
                    i_rate_batch = sess.run(
                        model.batch_ratings, {model.users: user_batch, model.pos_items: item_batch}
                    )
                else:
                    i_rate_batch = sess.run(
                        model.batch_ratings,
